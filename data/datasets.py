...
def recursively_read(rootdir, must_contain, exts=["png", "jpg", "JPEG", "jpeg"]):
    out = []
    class_labels = ['tvmonitor', 'chair']
    # class_labels = ['car', 'cat', 'chair', 'horse']
    print(rootdir)
...
def get_list(path, must_contain=''):
    ...
    for img in image_list[:len(image_list)] :
        ...
        if name not in dict:
            dict[name] = 0
        dict[name] += 1
    # return image_list[:len(image_list)//2]
    return image_list
...
class RealFakeDataset(Dataset):
    def __init__(self, opt):
        ...
        #     jitter_func = transforms.ColorJitter(
        #     brightness=tuple(opt.jitter_brightness_range.split(",")),
        #     contrast=tuple(opt.jitter_contrast_range.split(",")),
        #     saturation=tuple(opt.jitter_saturation_range.split(",")),
        #     hue=tuple(opt.jitter_hue_range.split(",")))
        # else :
...
def to_jpg(img, opt) :
    ...
    img = jpeg_from_key(img, 75, method)
