...
class RealFakeDataset_for_test(Dataset):
    def __init__(self,  dataset_path,
        ...
        real_path, fake_path, data_mode = dataset_path['real_path'], dataset_path['fake_path'], dataset_path['data_mode']
        self.is_resize = dataset_path['is_resize']
    ...
    def __getitem__(self, idx):
        ...
        if self.resize_scale is not None: 
            img = scale(img, self.resize_scale)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize_scale', type=float, default=None, help="0.9,0.8,0.7,0.6,0.5.     Used to test robustness of our model. Not apply if None")
    ...
    for dataset_path in (dataset_paths):
        set_seed()
        dataset = RealFakeDataset_for_test( dataset_path,
                                    opt.max_sample,
                                    opt.arch,
        ...
        ap, r_acc0, f_acc0, acc0, r_acc1, f_acc1, acc1, best_thres = validate(model, loader, find_thres=True, bs=opt.batch_size)

        rows.append([dataset_path['key'], f'{round(acc0*100, 2)}/{round(ap*100, 2)}', round(acc0*100, 2), round(ap*100, 2), round(acc1*100, 2)])
        mean_acc.append(acc0)
        mean_ap.append(ap)
    ...
    rows.append(['mean', f'{round(np.array(mean_acc).mean()*100, 2)}/{round(np.array(mean_ap).mean()*100, 2)}',
                 round(np.array(mean_acc).mean()*100, 2), round(np.array(mean_ap).mean()*100, 2)])
